...
use windows::core::{Error as HResultError, Interface, GUID, HSTRING};
...
unsafe fn get_attribute_string(attr: &IMFActivate, guid: GUID) -> String {
    ...
    let result = {
        ...
        String::from_utf16(slice).unwrap_or_else(|_| String::from_utf16_lossy(slice).into_owned())
    };
...
pub fn open(id: &str, config: CameraConfig) -> Result<(Box<dyn Stream>, DeviceControls)> {
    ...
    let controls = create_controls(stream.get_reader_arc());
        ...
        for index in 0..count {
            if let Ok(mt) = handler.GetMediaTypeByIndex(index) {
                if let Some((score, fmt)) = parse_media_type(&mt, config) {
                    // Update best format if this score is higher
                    if best_format
                        .as_ref()
...
unsafe fn parse_media_type(
    ...
    // Extract frame rate (numerator / denominator in 64-bit packed format)
    let fps = media_type
    ...
    // Calculate format score based on configuration preferences
...
/// Represents the negotiated video format configuration after format selection
///
/// This structure holds the final format parameters that were selected during
/// the format negotiation process. These parameters are used to configure both
/// the Media Foundation pipeline and the frame capture stream.
#[derive(Debug, Clone)]
pub struct NegotiatedFormat {
    ...
    pub height: u32,
    /// Pixel format used for video encoding (e.g., NV12, YUY2, MJPEG)
