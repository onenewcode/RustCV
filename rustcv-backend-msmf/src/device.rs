...
pub fn list_devices() -> Result<Vec<DeviceInfo>> {
    ...
    unsafe {
        ...
        let mut count: u32 = 0;
        MFEnumDeviceSources(&attributes, &mut pp_devices, &mut count).map_err(|e| {
            CameraError::Io(std::io::Error::new(
                std::io::ErrorKind::Other,
                e.to_string(),
            ))
        ...
        if count > 0 && !pp_devices.is_null() {
            ...
            for i in 0..count as usize {
                if let Some(attr) = devices_slice[i].as_mut().as_ref() {
                    ...
                    let device_id = get_attribute_string(
                        attr,
...
unsafe fn get_attribute_string(attr: &IMFActivate, guid: GUID) -> String {
    ...
    if attr
        .GetAllocatedString(&guid, &mut pwsz_value, &mut pcch_length)
        .is_ok()
        && !pwsz_value.is_null()
    {
...
pub fn open(id: &str, config: CameraConfig) -> Result<(Box<dyn Stream>, DeviceControls)> {
    unsafe {
        let mut attributes = None;
        MFCreateAttributes(&mut attributes, 2).map_err(|e| {
            CameraError::Io(std::io::Error::new(
                std::io::ErrorKind::Other,
                e.to_string(),
            ))
        ...
        let source = MFCreateDeviceSource(&attributes).map_err(|e| {
            CameraError::Io(std::io::Error::new(
                std::io::ErrorKind::Other,
                e.to_string(),
            ))
        })?;
        let source_reader = MFCreateSourceReaderFromMediaSource(&source, None).map_err(|e| {
            CameraError::Io(std::io::Error::new(
                std::io::ErrorKind::Other,
                e.to_string(),
            ))
        ...
        let media_type = MFCreateMediaType().map_err(|e| {
            CameraError::Io(std::io::Error::new(
                std::io::ErrorKind::Other,
                e.to_string(),
            ))
        ...
        media_type.SetGUID(&MF_MT_SUBTYPE, &mf_guid).map_err(|e| {
            CameraError::Io(std::io::Error::new(
                std::io::ErrorKind::Other,
                e.to_string(),
            ))
                    ...
                    if let Ok(major_type) = mt.GetGUID(&MF_MT_MAJOR_TYPE) {
                        if major_type == MFMediaType_Video {
                            if let Ok(subtype) = mt.GetGUID(&MF_MT_SUBTYPE) {
                                ...
                                let current_score =
                                    calculate_score(config, width, height, core_fmt);
